...
class Lists extends AdminController
{
	...
	private function renderLists()
	{
		...
		$keyword = $this->request->getGet('keyword_admin');
		$currentPage = $this->request->getGet('p', 1 , 'page');
		$limit = ($currentPage - 1) * $this->maxDataPerPage;
		$adminDomainArray = $this->adminDataSource->getAllAdminDomainByLimit( $limit, $this->maxDataPerPage, $keyword);
		...
		$this->sideBar = $this->menuBarRenderer->renderSideBarMenu('setting_admin');
		...
		$this->load->view('setting_administrator/lists', $this->viewVariable);
	}
}
?>