...
class Edit extends AdminController
{
    ...
    public function index()
    {
        ...
        if($this->request->getPost('process') == "edit_sales")
        {
    ...
    private function processEdit(AdminDomain $adminDomain)
    {
        ...
        $adm_star = $this->request->getPost('admin_star');
        ...
        if ($admin_image['name'] != '') {
            ...
            if ($admin_image['type'] != 'image/jpeg' AND $admin_image['type'] != "image/png") {
                ...
                $this->redirect('setting-administrator-sales/edit');
                return;
            }
        ...
        $sessionMessage['message'] = "Sales Data edited";
        $this->session->set('sessionAdminMessage', $sessionMessage);
    ...
    private function renderEdit(AdminDomain $adminDomain, ValidationErrors $validationErrors = NULL)
    {
		...
		$this->sideBar = $this->menuBarRenderer->renderSideBarMenu('administrator_sales');
		$this->headerBar = $this->menuBarRenderer->renderHeaderBar();

		// breadcrumb
		$this->breadcrumbArray = array(
            array('',$this->urlBuilder->build('dashboard'),'fa fa-home'),
            array('Sales',$this->urlBuilder->build('profile','',FALSE),''),
			array('Edit','','','')
        );
        // end breadcrumb
        ...
        $this->load->view('setting_administrator_sales/edit', $this->viewVariable);
    }
}
?>