...
class Add extends AdminController
{
	...
	private function addProcess()
	{
		...
		if ($admin_image['name'] != '') {
            ...
            $admin_image['name'] = "imgadmin".date('Y-m-d')."_".date('His').$ext;
             if ($admin_image['type'] != 'image/jpeg' AND $admin_image['type'] != "image/png") {
                 ...
                 $this->redirect('setting-administrator-sales/add');
                 return;
             }
        }
        ...
        $dataPost = array(
            ...
            'star' => $this->request->getPost('admin_star'),
		);
		...
		$this->redirect('setting-administrator-sales');
		return;
	}
	...
	private function renderAdd(ValidationErrors $validationErrors = NULL)
	{
		...
		$this->breadcrumbArray = array(
			...
			array('Sales',$this->urlBuilder->build('setting-administrator-sales','',FALSE),''),
		    array('Add','','')
		);
		...
		$this->load->view('setting_administrator_sales/add', $this->viewVariable);
	}
}